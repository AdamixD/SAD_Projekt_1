...
other_countries_inflation_avg <- data.frame(other_countries_inflation[1], "Kraje poza strefą Euro"=(rowMeans(other_countries_inflation[2:9], na.rm=TRUE)))
...
inflation_avg <- data.frame(
  eurozone_countries_inflation_avg[1],
...
inflation_avg_tidy |>
  ...
  geom_line(aes(color = origin), linewidth=1.2) +
  xlab("Data") + ylab("Wskaźnik inflacji") +
  ggtitle("Poziom inflacji na przestrzeni lat") +
    theme_economist()
